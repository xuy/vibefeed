...
test("one token both pushes to an owned lane and pulls that user's feed", () => {
  ...
  bus.ensureOwner("u1");
  bus.createChannel({ id: "mylane", title: "Mine", visibility: "private" }, auth.ownerId);
  bus.subscribe(auth.userId, "mylane"); // owner may subscribe to their own private lane

  // push (producer side: ownerId + scope)
